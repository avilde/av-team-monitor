...
class TeamMonitor {
	...
	processConfig (data) {
		this.cfg = data
		this.cfg.settings = this.cfg.settings.reduce((settings, setting) => {
		...
		this.cfg.teams = this.cfg.teams.reduce((teams, team) => {
			...
			team.members = team.members.reduce((members, member) => {
	...
	logConfig (logProject) {
		if (this.cfg) {
			console.log(
				`%c ${this.cfg.settings.shortName} %c ${this.cfg.settings.name} v${this.cfg.settings.version} `,
